...
The technique uses the diffraction pattern generated by shining an X-ray beam on a single crystal.
The pattern captured using this method reflects the symmetry of the crystal. Mathematically it can be expressed as the Fourier transform of
the electron density of the crystal.
The complex structure factor $F(arrow(h))$ is thus:
...
It can be represented by a two positions $(P_1, P_2)$.
Another defining feature of a pair is the pair vector $arrow(P_1 P_2)$.

Two pairs $(P_1, P_2)$ and $(P_1', P_2')$ are considered equivalent if there exist a transformation $cal(G) in frak(G)$ for which 
$(P_1, P_2) = (cal(G)P_1', cal(G)P_2')$ or $(P_2, P_1) = (cal(G)P_1', cal(G)P_2')$
...
In this two-dimensional example, it is apparent why we need to consider symmetries which move the end position of pair to the start position
of the untransformed pair as the pair with the pair vector $angle.l 1 0 angle.r$ and
the pair with pair vector $angle.l overline(1) 0 angle.r$ are equivalent even though
there is no symmetry element in the stabilizer of $P_1$ which would transform one to the other.
...
All coordinates used in the program are rational numbers implemented as a pair of `i32`,
which are always represented in reduced form and with a positive denominator.
Rational numbers were chosen instead of floating-point numbers to allow for exact calculations and comparisons of coordinates.
